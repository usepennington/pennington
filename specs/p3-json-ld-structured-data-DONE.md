...
- All URLs in JSON-LD must be absolute (use `CanonicalBaseUrl` from `DocSiteOptions` or `PenningtonOptions`)
...
## Key Files
- `src/Pennington.UI/Components/` or `src/Pennington.DocSite/Components/` — new `StructuredData` component
- `src/Pennington.DocSite/Components/Layout/MainLayout.razor` — include component in `<HeadContent>`
- `src/Pennington/Navigation/NavigationBuilder.cs` — breadcrumb data source
- `src/Pennington/FrontMatter/Capabilities.cs` — `IDescribable`, `IDateable` for article metadata
