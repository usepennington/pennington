...
_Two sentences. Frame the reader as someone who started from `AddDocSite` and needs to brand the chrome (head tags, footer HTML, one or two extra routed pages) without giving up the bundled layout, content pipeline, SPA navigation, and MonorailCSS wiring. Point out the escape hatches: if they want to replace the article island itself they should follow [Register an island renderer](xref:how-to.extensibility.island-renderer), and if they want a fundamentally different shell they should read [When is DocSite the right starting point?](xref:explanation.core.docsite-positioning) first._
...
- You have an existing Pennington site wired through `AddDocSite(...)` (see the [Getting Started tutorial](xref:tutorials.getting-started.first-site) if not).
- You are editing the `DocSiteOptions` factory you pass to `AddDocSite`, not the DocSite source — forking the template is out of scope (see [When is DocSite the right starting point?](xref:explanation.core.docsite-positioning)).
...
To copy a working setup, see [`examples/DocSiteChromeOverridesExample`](https://github.com/usepennington/pennington/tree/main/examples/DocSiteChromeOverridesExample) — `SiteChromeOverrides.cs` returns a populated `DocSiteOptions` exercising all four seams, `Components/ExtraHeadFragment.razor` is the component that backs the head-slot fragment, and `Components/ExtraPage.razor` is the routed `@page` component that proves `AdditionalRoutingAssemblies` widened the router. `Program.cs` runs the DocSite end-to-end against those overrides.
...
_Two sentences. `AddDocSite` takes a `Func<DocSiteOptions>` factory, so the simplest wiring is to pass the helper as a method reference and keep the host file short. The example's `Program.cs` runs exactly this shape end-to-end:_
...
_Follow [Register an island renderer](xref:how-to.extensibility.island-renderer) and register your `IIslandRenderer` with `IslandName => "content"` to displace the shipped `DocSiteArticleSlotRenderer`._
...
- Related how-to: [Register an island renderer](xref:how-to.extensibility.island-renderer)
