...
To add a custom fence syntax — a chart block, a plaintext wrapper, an xmldocid resolver — implement `ICodeBlockPreprocessor`. The preprocessor claims a fence language or `:modifier` suffix and returns pre-rendered HTML before the default highlighter chain runs. For line-level CSS classes on an otherwise normal code block, trailing-comment directives are the lighter-weight choice — see <xref:how-to.content-authoring.code-annotations>.
