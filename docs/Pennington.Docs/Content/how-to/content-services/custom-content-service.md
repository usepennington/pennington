...
Implement <xref:reference.api.i-content-service> as a sealed class. Cache the parsed records in a `Lazy<ImmutableList<T>>` so discovery and the TOC share one pass over the source. That cache holds for the service's lifetime, which has consequences for live edits — see the stale-data warning under [Register the service](#register-the-service).
...
- **Search facets** — the [`IHasSearchFacets`](xref:explanation.discovery.search) `channel` axis emits alongside the built-in `section`/`tag`/`area` dimensions.
- **JSON-LD** — the [`IHasStructuredData`](xref:how-to.rich-content.structured-data-custom-types) entity is injected into each release page's `<head>` automatically when `CanonicalBaseUrl` is set; no `<script>` to hand-write.
