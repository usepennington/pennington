...
Use this approach for fences tagged with a language token — a DSL, config format, or domain notation — that TextMateSharp does not cover, when styled output is the goal but authoring a full TextMate grammar is not. For line-level callouts on an already-supported language, see <xref:how-to.code-samples.code-annotations>. When the goal is transforming fence bodies rather than colouring tokens, see <xref:how-to.markdown-pipeline.code-block-preprocessor>.
