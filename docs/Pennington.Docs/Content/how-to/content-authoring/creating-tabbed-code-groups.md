...
- Reference `T:Pennington.Markdown.Extensions.Tabs.TabbedCodeBlocksExtension` which processes the document after parsing: it finds a `FencedCodeBlock` node with `tabs=true` in its arguments, then gathers all consecutive `FencedCodeBlock` siblings and wraps them in a `T:Pennington.Markdown.Extensions.Tabs.TabbedCodeBlock` container
- Reference `T:Pennington.Markdown.Extensions.Tabs.TabbedCodeBlockRenderer` which generates ARIA-compliant tab markup: `role="tablist"`, `role="tab"` buttons, and tab panels

### Key points
- Only consecutive code blocks are grouped; a paragraph between two `tabs=true` blocks breaks the group
- Reference `T:Pennington.Markdown.Extensions.Tabs.TabbedCodeBlockRenderOptions` for CSS class customization (`P:Pennington.Markdown.Extensions.Tabs.TabbedCodeBlockRenderOptions.ContainerCss`, `P:Pennington.Markdown.Extensions.Tabs.TabbedCodeBlockRenderOptions.TabListCss`, `P:Pennington.Markdown.Extensions.Tabs.TabbedCodeBlockRenderOptions.TabButtonCss`, `P:Pennington.Markdown.Extensions.Tabs.TabbedCodeBlockRenderOptions.TabPanelCss`)
...
### What to show
- Show that tab titles default to `M:Pennington.Markdown.Extensions.Tabs.LanguageNormalizer.GetLanguageName(System.String)` (e.g., "C#", "F#") unless overridden with `title='Custom'`
- Reference `M:Pennington.Markdown.Extensions.CodeBlockExtensions.GetArgumentPairs(Markdig.Syntax.FencedCodeBlock)` for parsing `tabs=true` and `title='Custom Title'` from the info string
- Example: ` ```csharp tabs=true title='Service Setup' ` produces a tab labeled "Service Setup" instead of "C#"

### Key points
- `T:Pennington.Markdown.Extensions.Tabs.LanguageNormalizer` maps language identifiers to human-friendly names (e.g., `csharp` becomes "C#", `fsharp` becomes "F#", `javascript` becomes "JavaScript")
...
- The `isInTabGroup` flag in `CodeHighlightRenderer` and `M:Pennington.Markdown.Extensions.CodeBlockHtmlBuilder.BuildHtml(System.String,Pennington.Markdown.Extensions.CodeHighlightRenderOptions,System.Boolean)` adjusts the wrapper CSS (no standalone container class when inside a tab)
