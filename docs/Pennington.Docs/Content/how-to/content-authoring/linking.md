...
To link from one page to another without hardcoding a URL that may break on rename or sub-path deploy, pick the link form that matches the target's relationship to the source. For `uid:`-based cross-references that survive arbitrary page moves, see <xref:how-to.content-authoring.cross-references>.
