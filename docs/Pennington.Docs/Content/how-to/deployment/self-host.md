...
This page covers serving an `output/` directory produced by `dotnet run -- build` from a server you control — a VPS running Nginx or a Windows host running IIS. When a managed static host is an option, see [Deploy to GitHub Pages](xref:how-to.deployment.github-pages) first; this page is for when that route is unavailable.
