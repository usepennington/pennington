...
Every content page is published as clean Markdown at its own URL with `.md` appended — for
example `/how-to/feeds/llms-txt.md`. Pages advertise that copy with a
...
- `/{any page path}.md` — that single page as front-matter-stamped Markdown (canonical URL,
...
- **Tutorials** (learn by building) — `/tutorials/getting-started/first-site.md`.
- **How-to** (solve one task) — for example `/how-to/feeds/llms-txt.md`; the full set is
...
- **Explanation** (understand the design) — `/explanation/core/mental-model.md`.
...
`/migrating-via-ai.md` first — it names the primitives to reuse and the dead ends to avoid.
