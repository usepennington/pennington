...
That is the whole getting-started arc. Building a site on `AddPennington` is the normal path, and you have now done every part of it. The [DocSite](xref:tutorials.docsite.scaffold) and [BlogSite](xref:tutorials.blogsite.scaffold) templates package this same kind of wiring — layout, navigation, search — into a single call for two specific shapes; reach for one when your site *is* a documentation site or a blog and you want to skip the assembly. [Connect a Roslyn project](xref:tutorials.beyond-basics.connect-roslyn) and the other [beyond-basics tutorials](xref:tutorials.beyond-basics.custom-razor-component) build on the host you just finished.
