...
- How-to: [Link to a page without breakage](xref:how-to.navigation.linking) — the authoring recipe for setting `uid:` and linking with `<xref:uid>` or `[text](xref:uid)`.
