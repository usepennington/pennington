...
The distinction that matters is that `DocSiteOptions` is not a mirror of `PenningtonOptions`. It is a curated subset plus a handful of DocSite-specific options, and the things it deliberately does not expose mark the edge of what the template covers.
...
To summarize the limits precisely: one markdown source registration, extendable via `ConfigurePennington` or by dropping a level; a fixed theme composition (`AddMonorailCss` plus the DocSite layout shell) that is tweakable but not swappable; and a fixed `App` component that can only be extended through additional routing assemblies, not replaced.
