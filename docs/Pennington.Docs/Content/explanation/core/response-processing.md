...
These integers are illustrative of the shipped chain, not a stable contract. What is guaranteed is the ordering *rule* the rest of this section explains — which rewriter must run before which — not the specific values, so a custom rewriter should pick an `Order` relative to the slot it needs rather than hard-code one of these. The <xref:reference.api.i-response-processor> reference carries the authoritative numbers.
