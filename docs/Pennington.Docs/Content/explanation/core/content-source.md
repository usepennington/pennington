...
A first instinct is to make `ContentSource` an interface with five implementations and dispatch through virtual methods. That solves the dispatch problem but introduces three drag points the union avoids:
...
- **The "no canonical body" cases get awkward.** `RedirectSource` carries a `TargetUrl` and nothing else; `EndpointSource` carries no payload at all. Modelling them as interface implementations forces them to satisfy the same shape as `MarkdownFileSource`, which actually has a path to read. The union lets each case carry exactly its own data.
...
## Why `RedirectSource` and `EndpointSource` exclude themselves from `sitemap.xml`

Both cases name a route, neither owns the canonical HTML for it. `RedirectSource` has no body at all — the response is a 30x to another URL. `EndpointSource` defers the body to a sibling `MapGet` whose response is the canonical HTML at that URL; from `SitemapService`'s perspective, the content service knows the route but does not own the page.
