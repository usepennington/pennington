...
Introduce the scenario: you want to inject a feedback widget (a `<script>` tag and a floating button `<div>`) into every page of Forge's documentation. Editing every Razor template is fragile and error-prone. Penn's response processor pipeline lets you transform HTML after rendering but before it reaches the browser — a clean cross-cutting concern.
...
### Key points
- Response processors are Penn's post-rendering transformation layer — they operate on the fully rendered HTML string
...
Walk through the three members of `T:Penn.Infrastructure.IResponseProcessor` at `:path:src/Penn/Infrastructure/IResponseProcessor.cs`.

### What to show
- `P:Penn.Infrastructure.IResponseProcessor.Order` — `int` that controls execution sequence. Lower values run first. The middleware sorts all processors by `Order` ascending before applying them
- `M:Penn.Infrastructure.IResponseProcessor.ShouldProcess(Microsoft.AspNetCore.Http.HttpContext)` — predicate that receives the `HttpContext` after the response has been generated. Return `true` to process this response, `false` to skip. Use this to filter by content type, status code, or request path
- `M:Penn.Infrastructure.IResponseProcessor.ProcessAsync(System.String,Microsoft.AspNetCore.Http.HttpContext)` — receives the full response body as a string and the `HttpContext`. Returns the (potentially modified) response body string. The output of one processor becomes the input of the next
...
### What to show
- The built-in processor chain registered in `M:Penn.Infrastructure.PennExtensions.AddPenn(Microsoft.Extensions.DependencyInjection.IServiceCollection,System.Action{Penn.Infrastructure.PennOptions})` at `:path:src/Penn/Infrastructure/PennExtensions.cs`:
  - `T:Penn.Infrastructure.XrefResolvingProcessor` — `Order: -10`. Resolves `xref:uid` links to actual URLs. Runs first so subsequent processors see resolved links
  - `T:Penn.Infrastructure.BaseUrlRewritingProcessor` — `Order: 0`. Rewrites root-relative URLs to include the configured base URL (for subdirectory deployments)
  - `T:Penn.Localization.LocaleLinkRewritingProcessor` — `Order: 50`. Rewrites internal links to include locale prefix for non-default locales
  - `T:Penn.Infrastructure.LiveReloadScriptProcessor` — `Order: 1000`. Injects the live reload WebSocket script (dev mode only)
  - `T:Penn.Infrastructure.DiagnosticOverlayProcessor` — `Order: 10000`. Injects the diagnostic overlay widget (dev mode only)
...
### What to show
- `T:Penn.Infrastructure.ResponseProcessingMiddleware` at `:path:src/Penn/Infrastructure/ResponseProcessingMiddleware.cs`:
  - `M:Penn.Infrastructure.ResponseProcessingMiddleware.InvokeAsync(Microsoft.AspNetCore.Http.HttpContext,System.Collections.Generic.IEnumerable{Penn.Infrastructure.IResponseProcessor})` captures the response body into a `MemoryStream`
...
- The middleware is registered by `M:Penn.Infrastructure.PennExtensions.UsePenn(Microsoft.AspNetCore.Builder.WebApplication)` via `app.UseMiddleware<ResponseProcessingMiddleware>()`

## Beat 5: Create the FeedbackWidgetProcessor

Implement `T:Penn.Infrastructure.IResponseProcessor` for the feedback widget injection.

### What to show
- Class declaration: `public sealed class FeedbackWidgetProcessor : IResponseProcessor`
- `P:Penn.Infrastructure.IResponseProcessor.Order` returns `500` — after xref resolution (-10), base URL rewriting (0), and locale link rewriting (50), but before live reload (1000)
- `M:Penn.Infrastructure.IResponseProcessor.ShouldProcess(Microsoft.AspNetCore.Http.HttpContext)` implementation:
...
- `M:Penn.Infrastructure.IResponseProcessor.ProcessAsync(System.String,Microsoft.AspNetCore.Http.HttpContext)` implementation:
  - Find `</body>` using `LastIndexOf` (same pattern as `T:Penn.Infrastructure.LiveReloadScriptProcessor` at `:path:src/Penn/Infrastructure/LiveReloadScriptProcessor.cs`)
...
- Use `LastIndexOf("</body>", StringComparison.OrdinalIgnoreCase)` for case-insensitive matching, consistent with `T:Penn.Infrastructure.LiveReloadScriptProcessor`
...
- Explain that `T:Penn.Infrastructure.ResponseProcessingMiddleware` receives `IEnumerable<IResponseProcessor>` via DI constructor injection — all registered `IResponseProcessor` implementations are automatically discovered and included in the chain
...
### Key points
- Static generation uses the same ASP.NET pipeline as the dev server — every request goes through `T:Penn.Infrastructure.ResponseProcessingMiddleware`
- This is a design guarantee: processors do not need conditional logic for dev vs. build
- If a processor should only run in dev mode (like `T:Penn.Infrastructure.LiveReloadScriptProcessor`), check `Environment.GetEnvironmentVariable("DOTNET_WATCH")` in `ShouldProcess`
...
### What to show
- Temporarily change `Order` to `-20` (before `T:Penn.Infrastructure.XrefResolvingProcessor` at -10). The processor now runs on HTML that still contains unresolved `xref:uid` placeholders — the feedback widget HTML itself could contain xref links that would never be resolved. Reset to `500`
- Temporarily change `Order` to `2000` (after `T:Penn.Infrastructure.LiveReloadScriptProcessor` at 1000). Now the feedback widget appears after the live reload script in the HTML — functionally fine, but demonstrates that ordering affects HTML position when both processors inject at the same `</body>` point
- Show the diagnostic headers: `T:Penn.Infrastructure.ResponseProcessingMiddleware` writes `X-Penn-Diagnostic` headers that contain content diagnostics (format: `Severity|Message|Source`) — these track content issues like unresolved xrefs, not processor execution order
