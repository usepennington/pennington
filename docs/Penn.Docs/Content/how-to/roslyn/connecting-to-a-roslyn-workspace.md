...
description: "Add Penn.Roslyn, configure RoslynOptions with SolutionPath and ProjectFilter, and use :xmldocid, :xmldocid,bodyonly, and :path code block modifiers to embed live code from your .NET solution"
uid: "penn.how-to.connecting-to-a-roslyn-workspace"
order: 10
---

## Beat 1: Install Penn.Roslyn and register the Roslyn services

The Penn.Roslyn package provides Roslyn-powered code highlighting and the ability to embed live source code from a .NET solution into documentation pages. This beat covers adding the NuGet package and calling the registration extension method.

### What to show
- Install the `Penn.Roslyn` NuGet package into the docs project. Show the package reference in the `.csproj` (note: the package targets `net11.0;net10.0` and depends on `Microsoft.CodeAnalysis.CSharp.Workspaces`, `Microsoft.CodeAnalysis.Workspaces.MSBuild`, `Microsoft.Build.Locator`, and `DiffPlex`). Reference `:path src/Penn.Roslyn/Penn.Roslyn.csproj`.
- In `Program.cs`, call `M:Penn.Roslyn.RoslynExtensions.AddPennRoslyn(Microsoft.Extensions.DependencyInjection.IServiceCollection,System.Action{Penn.Roslyn.RoslynOptions})` after `AddDocSite`. Show the minimal registration: `services.AddPennRoslyn(roslyn => { roslyn.SolutionPath = "../../Prism.slnx"; });`.
- Explain that `AddPennRoslyn` always registers `T:Penn.Roslyn.Highlighting.SyntaxHighlighter` (AdhocWorkspace-based Roslyn Classifier API highlighter) and `T:Penn.Roslyn.Highlighting.RoslynHighlighter` (which implements `T:Penn.Highlighting.ICodeHighlighter` at priority 100, beating TextMate at priority 50). When `SolutionPath` is set, it additionally registers `T:Penn.Roslyn.Workspace.SolutionWorkspaceService`, `T:Penn.Roslyn.Symbols.SymbolExtractionService`, and `T:Penn.Roslyn.Preprocessing.RoslynCodeBlockPreprocessor`.

### Key points
- `Penn.Roslyn` is a separate package so sites that do not need Roslyn analysis avoid the heavy MSBuild/Roslyn dependency tree
...
Detailed walkthrough of the two configuration properties on `T:Penn.Roslyn.RoslynOptions`: pointing at the solution file and optionally filtering which projects are analyzed.

### What to show
- Show the full `T:Penn.Roslyn.RoslynOptions` class: `P:Penn.Roslyn.RoslynOptions.SolutionPath` (nullable string, path to `.sln` or `.slnx`) and `P:Penn.Roslyn.RoslynOptions.ProjectFilter` (nullable `T:Penn.Roslyn.ProjectFilter`).
- Show the `T:Penn.Roslyn.ProjectFilter` record with its two properties: `P:Penn.Roslyn.ProjectFilter.IncludedProjects` (`HashSet<string>?`) and `P:Penn.Roslyn.ProjectFilter.ExcludedProjects` (`HashSet<string>?`). Demonstrate filtering to only Prism projects: `roslyn.ProjectFilter = new ProjectFilter { IncludedProjects = new HashSet<string> { "Prism.Generators", "Prism.Core" } };`.
- Explain how `T:Penn.Roslyn.Workspace.SolutionWorkspaceService` uses the filter in `M:Penn.Roslyn.Workspace.SolutionWorkspaceService.GetProjectsAsync(System.Func{Microsoft.CodeAnalysis.Project,System.Boolean})` via the private `ApplyProjectFilter` method: included projects are whitelisted, excluded projects are blacklisted, and both can be combined.
...
- Walk through the processing pipeline: `M:Penn.Roslyn.Preprocessing.RoslynCodeBlockPreprocessor.TryProcess(System.String,System.String)` calls `ParseLanguageId` to extract the modifier, then `ProcessXmlDocId` which splits the code block content into XML doc IDs (one per line), calls `M:Penn.Roslyn.Symbols.ISymbolExtractionService.ExtractCodeFragmentAsync(System.String,System.Boolean)` with `bodyOnly: false`, and highlights each fragment using `T:Penn.Roslyn.Highlighting.SyntaxHighlighter`.
...
### Key points
- The `T:Penn.Roslyn.Symbols.XmlDocIdNormalizer` strips namespace prefixes from parameter types so that both `M:Type.Method(System.String)` and `M:Type.Method(String)` resolve to the same symbol
- If a symbol is not found, a warning is emitted via `T:Penn.Diagnostics.DiagnosticContext` and an error comment is rendered in the code block
- The extracted code is fed through `M:Penn.Roslyn.Utilities.TextFormatter.NormalizeIndents(System.String)` to strip common leading whitespace
...
- `bodyonly` is passed through `M:Penn.Roslyn.Symbols.SymbolExtractionService.ExtractCodeFragmentAsync(System.String,System.Boolean)` as the second parameter
...
- Walk through the processing in the `ProcessPath` method of `T:Penn.Roslyn.Preprocessing.RoslynCodeBlockPreprocessor`: it validates the path (rejects `..` and rooted paths to prevent directory traversal), resolves the path relative to the solution directory (`Path.GetDirectoryName` of `P:Penn.Roslyn.RoslynOptions.SolutionPath`), reads the file, and highlights it via `T:Penn.Roslyn.Highlighting.SyntaxHighlighter`.
...
- Explain the file watching integration: `T:Penn.Roslyn.Workspace.SolutionWorkspaceService` subscribes to `T:Penn.Infrastructure.IFileWatcher` in its constructor. It registers watchers for `*.cs`, `*.csproj`, `*.sln`, and `*.slnx` files within the solution directory.
- Detail the smart update strategy: when a `.cs` file's content changes (`WatcherChangeTypes.Changed`), `M:Penn.Roslyn.Workspace.ISolutionWorkspaceService.UpdateDocument(System.String)` enqueues a deferred update. When a structural change occurs (file created, deleted, or renamed), `M:Penn.Roslyn.Workspace.ISolutionWorkspaceService.InvalidateSolution` forces a full solution reload. Deferred updates are applied in batch on the next `M:Penn.Roslyn.Workspace.ISolutionWorkspaceService.LoadSolutionAsync(System.String)` call.
- Mention that `T:Penn.Roslyn.Symbols.SymbolExtractionService` uses `AsyncLazy` for lazy symbol table loading. Note: `M:Penn.Roslyn.Symbols.ISymbolExtractionService.ClearCache` exists but is not currently called by `InvalidateSolution` — the symbol cache may become stale after structural changes until a full reload occurs. This is a known gap.
...
- Advise using XML doc comments (`///`) on the source code itself so that `P:Penn.Roslyn.Symbols.SymbolInfo.XmlDocumentation` is populated. This helps when tooling or future features consume the documentation metadata.
...
- The `T:Penn.Roslyn.Symbols.SymbolInfo` record captures the full context: `Symbol`, `Document`, `SyntaxNode`, `SourceText`, `TextSpan`, `XmlDocumentation`, and `Project`
- `T:Penn.Roslyn.Utilities.TextFormatter` normalizes indentation so extracted fragments render cleanly regardless of their nesting depth in the original source file
