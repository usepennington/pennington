...
- **`OutputGenerationService` parallel write race** surfaced when a URL was emitted both as a content-service `DiscoveredItem` *and* a `MapGet` route. Fix: serve release notes from the single catch-all `MapGet("/{*path}")`, not a dedicated `MapGet("/releases/")` — so only the content-service discovery populates the crawler queue.
- **`RazorIslandRenderer<T>` needs `ComponentRenderer` in DI.** AddPennington does not register it; `AddSpaNavigation()` does register the default `RenderContext` but not the renderer. Added `AddScoped<ComponentRenderer>()` explicitly.
