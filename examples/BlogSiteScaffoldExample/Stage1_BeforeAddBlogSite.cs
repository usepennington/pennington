...
/// <summary>
/// Stage 1 — the "bare Pennington" shape the reader arrives with from the
/// Getting Started tutorials. `AddPennington` registers the content pipeline
/// and a `MapGet` fallback walks <see cref="IContentService"/> instances to
/// render pages. No BlogSite template, no home page listing, no RSS feed,
/// no chrome. Tutorial prose extracts the body of <see cref="Run"/> via
/// <c>xmldocid,bodyonly</c>. This class is never instantiated.
/// </summary>
public static class Stage1
{
    /// <summary>The pre-BlogSite host — Pennington core with a plain fallback.</summary>
