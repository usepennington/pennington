...
/// and decorated with <c>section:</c> + <c>order:</c>. The subfolder is what
/// actually drives sidebar grouping: <c>NavigationBuilder</c> auto-creates a
/// "Getting Started" section node for the folder (kebab-case gets
/// title-cased). The <c>order: 10</c> key controls where the page sits
/// among its siblings inside that group — smaller numbers sort first, ties
/// break on title. The <c>section:</c> key is metadata on the TOC entry
/// (surfaced as <c>NavigationInfo.SectionName</c>); it is the folder, not
/// the key, that groups the sidebar.
/// </summary>
public static class Stage2
{
    /// <summary>The stage-2 markdown — section + order added, lives under getting-started/.</summary>
