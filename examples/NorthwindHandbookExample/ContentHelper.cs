...
public sealed class ContentHelper
{
    ...
    public async Task<List<(ContentRoute Route, IFrontMatter FrontMatter)>> GetAllPagesAsync()
    {
        var pages = new List<(ContentRoute, IFrontMatter)>();
    ...
    public async Task<(IFrontMatter FrontMatter, string Html)?> GetPageByUrlAsync(string url)
    {
        url = "/" + url.Trim('/');
