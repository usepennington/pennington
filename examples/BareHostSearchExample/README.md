...
- `scripts.js` self-initializes on load: it binds the click and the Ctrl/Cmd-K shortcut to the element with `id="search-input"` and reads `data-default-locale` (and, when present, `data-locales` / `data-base-url`) from `<body>`. It pulls in `DeweySearch.Web`'s client (`dewey-search.js`) on demand the first time the modal opens, so the layout never references that script directly.
