...
public sealed class ContentHelper
{
    ...
    public async Task<List<(ContentRoute Route, DocFrontMatter FrontMatter)>> GetAllPagesAsync()
    {
        var pages = new List<(ContentRoute, DocFrontMatter)>();
        foreach (var service in _services)
        {
            await foreach (var item in service.DiscoverAsync())
            {
                if (item.Source is MarkdownFileSource source)
                {
                    ...
                    var parsed = _parser.Parse<DocFrontMatter>(content);
                    var fm = parsed.Metadata ?? new DocFrontMatter();
    ...
    public async Task<(DocFrontMatter FrontMatter, string Html)?> GetPageByUrlAsync(string url)
    {
        url = "/" + url.Trim('/');
