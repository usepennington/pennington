...
- **Metadata enricher** — `GitTimestampEnricher` merges a `git_last_modified` date into `ParsedItem.Derived` (read from the source file's timestamp). `LastModified.razor` is the consumer: an Mdazor component that reads `Context["Derived"]` and renders the date; `Content/metadata-demo.md` exercises both, and the key surfaces in `/metadata-demo/index.md`. (`how-to/markdown-pipeline/metadata-enrichers.md`)
...
- **Artifact service** — `RobotsTxtContentService` implements `IArtifactContentService`: an `ExactClaim` on `/robots.txt`, one resolver producing the bytes for both surfaces. The artifact router serves it live under `dotnet run`, and `dotnet run -- build` writes the same bytes to `output/robots.txt`. (`how-to/content-services/emit-generated-artifacts.md`)
