...
| `BareHostSearchExample` | Light up the Pennington.UI search modal on a bare (non-DocSite) host — reference `Pennington.UI`, load `scripts.js` (it pulls in `dewey-search.js` on demand when search first opens), add an `id="search-input"` trigger; styled by the `PenningtonApplies` safelist. Mounts the `_shared/Bramble` corpus (blog excluded). | `how-to/discovery/search-on-a-bare-host.md` |
...
| `BeyondCookFormatExample` | Register a custom file format — Cooklang `.cook` recipes — via `AddContentFormat<CookFrontMatter>("cook", …)`. The renderer is a `RazorContentRenderer<RecipeView>` subclass that binds the `CooklangSharp` model to a `RecipeView.razor` component (the base owns HtmlRenderer dispatch + outline); the parser splits front matter. Recipes at `/recipes/{slug}/` flow through the same discover → parse → render pipeline as a markdown landing page, exercising the multi-format dispatch seam (format-tagged `FileSource`, dispatching parser/renderer). | `how-to/content-services/custom-content-format.md` |
...
| `BeyondTreeSitterExample` | `AddTreeSitter` against a `Samples/` folder — `<lang>:symbol` fences extract declarations by name path (`Type.Member`) across Python/Rust/Go/TypeScript, plus `,bodyonly` and whole-file forms. | _(reference for `Pennington.TreeSitter`)_ |
...
| `BlogSiteScaffoldExample` | Smallest BlogSite — `AddBlogSite` / `UseBlogSite` / `RunBlogSiteAsync` with one post under `Content/Blog/`, plus a root `Content/404.md` not-found body. | `tutorials/blogsite/scaffold.md`, `reference/blogsite/routes.md`, `how-to/pages/not-found-page.md` |
| `BlogSiteSocialCardsExample` | Generated OpenGraph social cards via `BlogSiteOptions.SocialCards` — host-owned `Render` hook with a dependency-free PNG painter, per-post cards plus the home page's site-identity card. | `how-to/feeds/social-cards.md` |
| `CustomFrontMatterRazorPageExample` | Bare `AddPennington` host whose catch-all `@page` reads a custom `ApiFrontMatter` record via `IPageResolver.ResolveAsync<ApiFrontMatter>` — typed `namespace`/`stability` keys with no cast or Mdazor context. | `how-to/pages/front-matter.md` |
...
| `DocSiteChromeOverridesExample` | Override DocSite chrome via `DocSiteOptions` + head-slot fragment + custom routed `@page` + `AdditionalRoutingAssemblies`, plus a `Components/NotFound.razor` 404 body and a brand `ColorScheme` (`ColorTheme.Orchid`) whose auto-coordinated palette the home page shows off via a `BrandPalette` Mdazor component. | `how-to/response-pipeline/override-docsite-components.md`, `how-to/pages/not-found-page.md` |
...
| `DocSiteScaffoldExample` | Smallest DocSite — `AddDocSite` / `UseDocSite` / `RunDocSiteAsync` with a `Content/guides/` folder that auto-groups in the sidebar (no areas), plus a root `Content/404.md` not-found body. | `tutorials/docsite/scaffold.md`, `reference/host/extensions.md`, `how-to/pages/not-found-page.md` |
...
| `EventMicrositeExample` | Conference microsite on a bare `AddPennington` host — `AddDataFile<T>` typed hot-reloading YAML data (`data/sponsors.yml`, `data/schedule.yml` via `IDataFiles.Get<T>`) plus two `AddTaxonomy<TalkFrontMatter,string>` axes (`/topic/` single-valued, `/tag/` multi-valued) over markdown talks, mounted with one `MapTaxonomy` call. | _(not yet fenced from docs; intended: `how-to/content-services/{data-files,taxonomy}.md`)_ |
| `ExtensibilityLabExample` | Bare-host lab exercising every Pennington extension seam in one project: Markdig extension/inline parser (`WikiLinkExtension` via `ConfigureMarkdownPipeline`), custom highlighter, code-block preprocessor, custom `IContentService`, artifact service (`IArtifactContentService` robots.txt), response processor, diagnostics processor, HTML rewriter, MonorailCSS customization, llms.txt opt-in, tabbed-code class override. | `how-to/markdown-pipeline/{markdig-extension,custom-highlighter,code-block-preprocessor}.md`, `how-to/content-services/{custom-content-service,emit-generated-artifacts}.md`, `how-to/response-pipeline/{response-processor,html-rewriter}.md`, `how-to/theming/monorail-css.md`, `how-to/feeds/llms-txt.md`, `how-to/code-samples/tabbed-code.md`, `explanation/positioning/docsite-positioning.md`, `reference/diagnostics/request-context.md` |
...
| `PaginatedListingExample` | Bare host with a paginated `/articles` listing over a folder of markdown articles — core's `PagedList<T>`, an `ArticlesPage` Razor `@page` (canonical + `/articles/page/{Page:int}`), an `ArticleResolver` that slices the articles, and an `ArticleListingContentService` that emits the numbered routes (a meta-service marked `IMetaContentService` that resolves siblings on demand and filters them with `SourceServices()` to avoid recursion, mirroring `SocialCardContentService`). | `how-to/discovery/pagination.md` |
