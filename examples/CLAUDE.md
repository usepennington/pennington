...
| `PaginatedListingExample` | Bare host with a paginated `/articles` listing over a folder of markdown articles — a `PagedList<T>` record, an `ArticlesPage` Razor `@page` (canonical + `/articles/page/{Page:int}`), an `ArticleResolver` that slices the articles, and an `ArticleListingContentService` that emits the numbered routes (resolves siblings on demand to avoid a DI cycle, mirroring `SocialCardContentService`). | `how-to/discovery/pagination.md` |
