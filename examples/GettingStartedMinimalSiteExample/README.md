...
The smallest viable Pennington host: `AddPennington` plus `AddMarkdownContent<DocFrontMatter>`, a single catch-all `MapGet` that resolves a URL through `IContentService` / `IContentParser` / `IContentRenderer`, and `RunOrBuildAsync` to switch between live serve and static build.

## Concepts

- `AddPennington` / `UsePennington` minimum wiring
- One markdown source rooted at `Content/` mapped to `/`
- Manual catch-all rendering loop (the shape DocSite later replaces — the tutorial's "From `MapGet` to `UseDocSite`" section shows the two side-by-side)
