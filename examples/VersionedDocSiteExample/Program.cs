...
Wiring.AddVersionedAreas(builder);
Wiring.AddVersionedApiReferences(builder);
...
static class Wiring
{
    // One area per documented version. ContentArea slugs map to top-level
    // folders under Content/, so files at Content/v1/foo.md route to /v1/foo
    // and the sidebar renders an area selector that doubles as a version switcher.
    ...
    // One metadata provider plus one AddApiReference per version. Only one version
    // of any given assembly can load through the default load context, so
    // FromPackageReference is reserved for the active (v2) reference; v1 lives in
    // the NuGet cache via <PackageDownload> in the .csproj, so AssemblyFiles points
    // at its cached path. RoutePrefix nests each tree inside its version area.
