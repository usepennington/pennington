        ...
        Func<IServiceProvider, MonorailCssOptions>? optionFactory = null)
    {
        ...
        services.AddSingleton<IConfigureOptions<MonorailDiscoveryOptions>>(sp =>
            new ConfigureNamedOptions<MonorailDiscoveryOptions>(Options.DefaultName, opts =>
            {
                ...
                // IL discovery scans every non-BCL assembly for class-shaped strings. The
                // MonorailCss framework only contributes noise — it ships utility *template*
                // strings (e.g. "bg-{color}-500"). Excluding it keeps the discovered set
