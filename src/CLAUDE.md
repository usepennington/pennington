...
## Folder metadata (`_meta.yml`)
Per-folder YAML sidecar that lives in the content tree alongside markdown. Carries the folder's display title, its sort position in the parent nav level, and an optional `llms:` block opting the subtree into `llms.txt` generation (the consolidation point for what was once `_llms.yaml`).

- `MarkdownContentService` discovers `_meta.yml` files and surfaces them via `IFolderMetadataProvider`.
