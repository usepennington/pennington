...
/// <c>&lt;dialog class="beck-lightbox"&gt;</c> holding a clone of the clicked diagram's SVG —
...
internal sealed class BeckZoomHeadContributor : IHeadContributor
{
    ...
    /// <c>[data-theme="dark"]</c> — the same signal the Beck SVG itself uses.
    /// </summary>
    internal const string Assets = """
        ...
        .beck-lightbox::backdrop{background:rgb(0 0 0/.25);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)}
        .beck-lightbox .beck-svg{width:auto;height:auto;max-width:94vw;max-height:90vh}
        .beck-lightbox-close{position:fixed;top:18px;right:18px;display:flex;align-items:center;justify-content:center;width:38px;height:38px;padding:0;cursor:pointer;border:1px solid rgb(255 255 255/.25);border-radius:9999px;background:rgb(0 0 0/.35);color:#fff;transition:background-color .15s}
        .beck-lightbox-close:hover{background:rgb(0 0 0/.55)}
        </style>
